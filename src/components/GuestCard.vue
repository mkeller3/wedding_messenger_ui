...
<v-card>
    <v-card-text>
        ...
        <div class="guest-info" v-if="guest.full_address"><v-icon>mdi-home-map-marker</v-icon> {{guest.full_address}}</div>
...
export default {
    ...
    methods: {
        deleteGuest: function(guest){
            axios.delete(`http://192.168.50.31:8000/api/v1/guest/?id=${guest.id}`, {
                headers: {
